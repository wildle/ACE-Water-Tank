...
\subsection{Water tank parameter}
The hydraulic outflow behaviour of the reservoirs is characterized by 
an effective flow coefficient, which is identified experimentally using 
steady-state measurements. System operation is constrained by the 
maximum admissible pump voltage and by the physical limits of the 
reservoir heights. $a_{1}$ can be calculated by:
...
\begin{equation}
	b = \frac{1}{\rho A_t} = \frac{1}{\qty{1000}{kg/m^3} \times \qty{50e-4}{m^2}} = \qty{0.2}{\meter\per\kilogram}
\end{equation}
...
performed by recording the sensor output voltage at known water heights as depicted in figure \ref{fig:sensorcal} and identifying the
...
\begin{figure}[H]
	...
	\caption[Diagram to determine sensor gain and offset]{Diagram of the linear fit of the measurements to determine the sensor gain and offset}
	\label{fig:sensorcal}
\end{figure}
