...
Based on the characteristic relation given in \cite{ACE_WT_Lab}, the pump is approximated by a
static linear model
...
regression of the diagram, given in the laboratory script \cite{ACE_WT_Lab}, depicted in figure \ref{fig:pumpcurve}. Linear regression yields $K_{\mathrm{pump}} = \qty{0.0167}{\kilogram\per\second\per\volt}$ and
$U_0 = \qty{0.25}{\volt}$.
...
\begin{figure}[H]
	...
	\caption{Pump characteristic relation and linear fit \cite{ACE_WT_Lab}}
	\label{fig:pumpcurve}  
\end{figure}
...
reservoirs. Under steady-state conditions, the outflow coefficient of the upper reservoir is obtained as
\begin{equation}
...
\begin{equation}
	a_{1} = \qty{0.0574}{\meter\tothe{1/2}\per\second}.
\end{equation}
...
resulting in
\begin{equation}
	a_{2} = \qty{0.0524}{\meter\tothe{1/2}\per\second}.
\end{equation}
